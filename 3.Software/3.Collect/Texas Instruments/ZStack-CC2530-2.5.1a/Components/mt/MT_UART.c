...
void MT_UartInit ()
{
  ...
  uartConfig.intEnable            = TRUE;                       //ÖÐ¶Ï
#if defined (ZTOOL_P1) || defined (ZTOOL_P2)
  uartConfig.callBackFunc         = MT_UartProcessZToolData;    //»Øµ÷º¯Êý
...
void MT_UartProcessZToolData ( uint8 port, uint8 event )
{
 ...
 (void)event;
  while (Hal_UART_RxBufLen(port)){   //Ñ­»·½« DMA ÖÐ½ÓÊÕµ½µÄ´®¿ÚÊý¾Ý¶Á³öÀ´£¬²¢·ÅÈë»º³åÇø rx_buf£»
        HalUARTRead(port, &rx_buf[rx_len], 1);
        rx_len++;
    }
    if (rx_len != 0) {          //È·±£»º³åÇøÖÐÓÐÊý¾Ý£»
        pMsg = (mtOSALSerialData_t *)osal_msg_allocate( sizeof( mtOSALSerialData_t ) + rx_len + 1);//ÎªÏûÏ¢·ÖÅäÄÚ´æ¿Õ¼ä£»
        pMsg->hdr.event = CMD_SERIAL_MSG;       //Ö¸¶¨ÁËÏûÏ¢µÄÀàÐÍ£¬½«´®¿ÚÊý¾Ý¸´ÖÆµ½ÏûÏ¢µÄÊý¾ÝÇø£¬Êý¾ÝµÄ¸ñÊ½Îª£ºÊý¾Ý³¤¶È+Êý¾Ý
        pMsg->msg = (uint8*)(pMsg + 1);
        pMsg->msg[0] = rx_len;
        for (i = 0; i < rx_len; i++){
          pMsg->msg[i + 1] = rx_buf[i];
        }
    osal_msg_send( App_TaskID, (byte *)pMsg );//½«ÏûÏ¢·¢ËÍ¸øÖ¸¶¨ task_id µÄÈÎÎñ¡£
    }
}
